...
int get_block(int ql, int qr) {
    if (ql > qr) {
        return 0;
    }
    int ans = 0;
    for (ql += R - 1, qr += R - 1; ql <= qr; ql >>= 1, qr >>= 1) {
        if (ql & 1) ans = max(ans, t_block[ql++]);
        if (~qr & 1) ans = max(ans, t_block[qr--]);
    }
    return ans;
}

int get_tail(int ql, int qr) {
    if (ql > qr) {
        return 0;
    }
    int ans = 0;
    for (ql += R - 1, qr += R - 1; ql <= qr; ql >>= 1, qr >>= 1) {
        if (ql & 1) ans = max(ans, t_tail[ql++]);
        if (~qr & 1) ans = max(ans, t_tail[qr--]);
    }
    return ans;
}
...
int main() {
    ...
    auto get_qu = [&](int l, int r) -> int {
        ...
        if (l > r) return ans;
        if (l <= tail.size()) {
            ans = max(ans, get_tail(l, min((int)tail.size(), r)));
            l = tail.size() + 1;
            if (l > r) return ans;
        }
        if (r - l + 1 >= block.size()) {
            ans = max(ans, get_block(1, block.size()));
            return ans;
        }
        int ql = (l - tail.size() - 1) % block.size() + 1;
        int qr = (r - tail.size() - 1) % block.size() + 1;
        if (ql <= qr) {
            ans = max(ans, get_block(ql, qr));
        } else {
            ans = max(ans, get_block(ql, (int)block.size()));
            ans = max(ans, get_block(1, qr));
        }
        return ans;
    };
    ll ans = 0;
