...
namespace SegTree {
    ...
    int Get(int i) {
        int ans = 0;
        for (i += N; i >= 1; i >>= 1) if (!t[i].empty()) ans = max(ans, *t[i].rbegin());
        return ans;
    }
...
void solve() {
    ...
    for(int to = 1; to <= n; ++to) {
        for(auto &event : events[to]) {
            if (event.type == ADD) {
                SegTree::Add(tin[event.vid], tout[event.vid], c[event.vid]);
                continue;
            } else if (event.type == REMOVE) {
                SegTree::Delete(tin[event.vid], tout[event.vid], c[event.vid]);
                continue;
            }
            // query
