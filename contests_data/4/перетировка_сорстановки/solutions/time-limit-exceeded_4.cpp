...
long long hsh(const vector<int> &a) {
    long long sum = 0;
    long long pw = 1;
    for (int i = 0; i < (int) a.size(); ++i) {
        sum += a[i] * pw % mod;
        pw = (long long) pw * Z % mod;
    }
    return sum % mod;
}

bool naive_sort(int l, int r, int k, vector<int> &a) {
    unordered_map<int, pair<vector<int>, pair<int, int>>> par(20000);
    ...
    while (st.size() && !par.count(hsh(sorted))) {
        auto tmp = st.front();
        st.pop();
        for (int i = 0; i + k <= (int) tmp.size(); ++i) {
            ...
            swap(z[ind1], z[ind2]);
            if (!par.count(hsh(z))) {
                par[hsh(z)] = {tmp, {l + i, l + i + k}};
                st.push(z);
            }
        }
    }
    if (par.count(hsh(sorted))) {
        for (int i = l; i < r; ++i) {
            a[i] = sorted[i - l];
        }
        vector<pair<int, int>> ops;
        while (sorted != start) {
            auto z = par[hsh(sorted)];
...
void solve() {
    ...
    for (int i = 0; i < 1'000; ++i) {
        int ind = rnd() % (n - k);
        int ind1 = min_element(a.begin() + ind, a.begin() + ind + k) - a.begin();
        int ind2 = max_element(a.begin() + ind, a.begin() + ind + k) - a.begin();
        swap(a[ind1], a[ind2]);
        ans.emplace_back(ind, ind + k);
    }
